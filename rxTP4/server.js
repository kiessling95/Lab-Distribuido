...
io.on("connection", (socket) => {
  ...
  // Quien ingresa recibe la lista de conectados
  socket.emit("listaConectados", [ ...clientes.values() ]);

  socket.on("name", (name) => {
    //Almaceno información ( socket.id, nickname)
    nickname.set(name, socket.id);
    clientes.set(socket.id, name);
    console.log(`Se conecto nuevo cliente nickname -> ${name}`);
    socket.broadcast.emit("nuevoCliente", name);
  });
  ...
  socket.on("messageTo", (msg) => {
    ...
    if (destinatario) {
      socket
        .to(destinatario)
        .emit("message", { message: msg.message, from: emisor });
    } else {
  ...
  socket.on("disconnect", () => {
    ...
    socket.broadcast.emit("clienteDejaChat", userName);
